...
public class ImageContainer {
    ...
    /**
     * Create a new image container, without opening any image file. Image sizes
     * required for full compatibility with other constructors, and cannot obtain
     * such information otherwise without opening image files.
     *
     * @param image_id Image identifier
     * @param image_file Image file
     ...
     *@see BufferedImage
     */
    public BufferedImage getImage(String relative) {

        try {
            if (this.width == -1) {
                BufferedImage image = JAI.create("url", new File(relative+image_file.getPath()).toURI().toURL()).getAsBufferedImage();
                this.width = image.getWidth();
                this.height = image.getHeight();
                return image;
            }
            return JAI.create("url", new File(relative+image_file.getPath()).toURI().toURL()).getAsBufferedImage();
        } catch (MalformedURLException ex) {
            Logger.getLogger(ImageContainer.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    ...
    /**Get the image light position values.
    *@return a float array with the light position values.
    */
    public float[] getLp() {
        float[] lp_values = new float[3];
        if(lp!=null){
            lp_values[0]= lp[0];
            lp_values[1]= lp[1];
            lp_values[2]= lp[2];
    ...
    /**Set the image light position values.
    *@param lp a float array with the light position to the image.
    */
    public void setLp(float[] lp_values) {
        lp = new float[3];
        lp[0] = lp_values[0];
        lp[1] = lp_values[1];
        lp[2] = lp_values[2];
