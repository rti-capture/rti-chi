 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_calib.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* calib.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* calib.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* calib (application created by MATLAB Compiler)
		* run_calib.sh (shell script that can be run 
	...
	All Platforms:            
		* calib.dll (shared library, file extension 
		  varies by platform)
		* calib.h (library header file)
		* calib.ctf (Component Technology File archive,
	...
	* calib.xml (documentation files)
	* calib.pdb (program database file - if 
	  DEBUG option selected)
	* calib.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* calib.jar (Java class files and compiled M-code)
	...
	* calib.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* calib.xla (Optionally created and placed in
