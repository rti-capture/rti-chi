...
LightControl::LightControl(QWidget *parent, int size): QWidget(parent),
	width(size),
	height(size),
	xFocus(size/2),
	yFocus(size/2),
...
void LightControl::paintEvent(QPaintEvent *event)
{
...
void LightControl::mousePressEvent(QMouseEvent *event)
{
	if (!enabled) return;
	timer->start(40);
...
void LightControl::mouseMoveEvent(QMouseEvent *event)
{
	if (!enabled) return;
	if (dragging)
	{
...
void LightControl::mouseReleaseEvent(QMouseEvent *event)
{
	if (!enabled) return;
	timer->stop();
	if (event->button() == Qt::LeftButton)
...
void LightControl::setEnabled(bool value)
{
	enabled = value;
	update();
}
...
void LightControl::moveLightPosition(float offx, float offy)
{
	if (!enabled) return;
